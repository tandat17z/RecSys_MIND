...
from layers import Attention, ScaledDotProduct_CandidateAttention
...
class NewsEncoder(nn.Module):
    def __init__(self, config: Config):
        ...
        with open('cospus/word_embedding-' + str(config.word_threshold) + '-' + str(config.word_embedding_dim) + '-' + config.tokenizer + '-' + str(config.max_title_length) + '-' + str(config.max_abstract_length) + '-' + config.dataset + '.pkl', 'rb') as word_embedding_f:
...
#         with open('cospus/entity_embedding-%s.pkl' % config.dataset, 'rb') as entity_embedding_f:
#             self.entity_embedding.weight.data.copy_(pickle.load(entity_embedding_f))
#         with open('cospus/context_embedding-%s.pkl' % config.dataset, 'rb') as context_embedding_f:
